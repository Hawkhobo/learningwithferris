...
// Passed in as reference, need to dereference for assignment (or use) of actualy value
// this one is probably best, but depends on desired behavior
pub fn double_v2(n: &i32) -> i32 {
    2 * *n
}

// since mutable, can double n
// Passed in as reference, need to dereference for assignment (or use) of actualy value
pub fn double_v3(n: &mut i32) {
    // double n in place
    *n *= 2
    //n <- just return result of assignment
}
